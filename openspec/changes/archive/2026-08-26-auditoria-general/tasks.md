...
- [x] 4.1 RED: extend `apps/api/src/auth/service.test.ts` — wrong current password → `INVALID_CURRENT_PASSWORD`, no writes, no audit row; success → `updatePassword`, `deleteOthers`, `auditoria.record` called in that order, all inside one `uow.run`; a throwing audit stub propagates and the fake `run` reports it did not commit. Confirmed RED first: 2/11 failed (`TypeError: Cannot read properties of undefined (reading 'updatePassword')`) — the wrong-password test already passed under the old signature since it never reaches repos, which is expected and documents an existing invariant, not new behavior
- [x] 4.2 GREEN `apps/api/src/auth/service.ts` — `changePassword(uow, input)` replaces `changePassword(repos, input)`; `verifyPassword`/`hashPassword` stay outside the transaction (D3); `updatePassword` → `deleteOthers` → `recordAudit` run inside `uow.run` (D1, D4, D8); snapshot `datosPrevios: { debeCambiarPassword: usuario.debeCambiarPassword }` → `datosPosteriores: { debeCambiarPassword: false }` (D7 non-symmetric case — the prior value is read from the actual `usuario`, not hardcoded). 11/11 passing after
- [x] 4.3 GREEN `apps/api/src/routes/auth.ts` — one call expression: `changePassword(app.uow, { usuario, sessionId, currentPassword, newPassword })`; no schema/DTO change (D16)
- [x] 4.4 RED: extend `apps/api/src/routes/auth.integration.test.ts` (Docker Postgres) — `POST /api/auth/password` writes exactly one `auditoria` row with no `hash_contrasena` in either snapshot; with the audit insert forced to fail (a `UnitOfWork` running a real `db.transaction` whose `auditoria.record` always throws), password unchanged, sessions NOT revoked, response is `500 AUDIT_WRITE_FAILED`
- [x] 4.5 GREEN — confirmed 4.4 passes against 4.2/4.3 immediately (6/6, was 4/4), no further production code needed
- [x] 4.6 Ran `pnpm test:integration` myself (agent-executed, Docker Postgres running, per this batch's explicit instruction overriding the original MANUAL note): 5 files / 22 tests passed (was 5/20 before this slice)
- [x] 4.7 Verify on the PR branch: `pnpm lint` (initially failed on import-order in `auth.integration.test.ts`, fixed with `pnpm biome check --write`, pure import reordering confirmed by reading the diff), `pnpm typecheck`, `pnpm contract:check` (byte-identical), `pnpm -r test` — all exit 0
