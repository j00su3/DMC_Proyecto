...
`POST /api/auth/password` MUST be an authenticated route (not `config: { auth: false }`) accepting `{ currentPassword, newPassword }`, verifying `currentPassword` against the caller's `hash_contrasena` via the existing argon2 module, and on success updating `hash_contrasena` to the hash of `newPassword` and setting `debe_cambiar_password` to `false`.
...
- THEN the response is `200`, `hash_contrasena` is updated, and `debe_cambiar_password` becomes `false`

#### Scenario: Wrong current password
- GIVEN an authenticated user submits an incorrect current password
- WHEN `POST /api/auth/password` is called
- THEN the response is `400 { error: { code: "INVALID_CURRENT_PASSWORD" } }` and no fields are updated
...
- WHEN `POST /api/auth/password` is called
- THEN the response is `400 { error: { code: "VALIDATION_ERROR" } }`
