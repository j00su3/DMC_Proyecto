...
- [ ] 2.7 MANUAL, **still open for NEON**. Apply `drizzle/0002_fat_whizzer.sql` to the live Neon database from `apps/api` with `pnpm db:migrate` (ADR-0010), at any point before S3 deploys. `pnpm db:migrate` fails from an agent's hands because `drizzle-kit` reads `DATABASE_URL` from a `.env*` file tooling must never touch — expected, not a bug.
