...
- [ ] 2.7 MANUAL (user, local/external action — not executable by the agent, no `.env*` access). Apply `drizzle/0002_fat_whizzer.sql` to the live Neon database, run from `apps/api`: `pnpm db:migrate` (ADR-0010), at any point before S2b deploys. Note: `pnpm db:migrate` will fail from an agent's hands because `drizzle-kit` needs `DATABASE_URL` from a `.env*` file the agent must never touch — this is expected, not a bug
