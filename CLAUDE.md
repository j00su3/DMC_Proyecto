...
report can never contain its own sha. Verify at a revision, then commit the report on top of
it. Cycles still in planning are not gated, and neither are cycles the PR's branch has
nothing to do with — unless the hook cannot resolve that branch at all, in which case it
checks every closing cycle rather than assume the PR is unrelated.
**Do not work around a refusal** — it is reporting a false statement that is still written down. Fix the
