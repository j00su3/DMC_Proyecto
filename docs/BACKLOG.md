...
| 2.5 | Seudonimizar el correo en las instantáneas de auditoría | Cierra **SEC-012** (LOW, gobierno del dato). La identidad del actor ya vive en `auditoria.usuario_id`, un UUID sin significado propio que basta para el no repudio. Decisión de producto cerrada en `PRD.md` § Supuestos y riesgos abiertos: rastro **permanente y sin datos personales**, descartada la ventana de retención con purga porque le pondría vencimiento al no repudio. **Evaluado el 2026-08-30 y frenado a propósito**: mover `email` a `excludedFields` deja las filas de un cambio de-solo-correo con ambas instantáneas vacías — registran que algo cambió sin poder decir qué. **Resuelto el 2026-09-01 por el propietario: seudonimizar, no omitir.** `email` permanece en `auditableFields` (`apps/api/src/auditoria/fields.ts`), pero `recordAudit` (`apps/api/src/auditoria/service.ts`) lo sustituye en ambas instantáneas por un HMAC-SHA256 determinístico (`hmac-sha256:<hex>`), con clave `COOKIE_SECRET` reutilizada y una etiqueta de separación de dominio propia — nunca `crypto.createHash` a secas, porque un hash sin clave es reversible por diccionario contra patrones de correo comunes. Determinístico ⇒ el mismo correo siempre produce el mismo seudónimo, así que un cambio de-solo-correo sigue mostrando un diff visible entre ambas instantáneas en vez de dos filas vacías o idénticas | #2.2 | ✅ Hecho |
...
| 10 | Motor de alertas | Tabla Alerta; interfaz `EvaluadorDeAlertas` + `ReglasUmbral` dentro de la transacción del movimiento protegido por `SAVEPOINT` (C1); creación por cruce de umbral, de-duplicación por producto+tipo, auto-resolución de stock_bajo/quiebre, resolución manual de discrepancia (A10); alerta de discrepancia desde ajustes marcados (A9); vista de alertas + polling del conteo en la SPA | #6, #7 | ✅ Archivado |
| 11 | Sugerencia de reposición | Heurística dentro de `ReglasUmbral`: promedio diario de salidas de 30 días, cobertura < 14 días, mínimo 7 días de historia, promedio 0 nunca sugiere (S7); resolución manual por el encargado. **Cerrado y archivado el 2026-09-03** (PRs #161 foundation `resumenRotacion` + #162 evaluador/gates/wiring, ambos mergeados a `main`). Sin tabla nueva, sin migración: reutiliza el `alertas` table y el mecanismo SAVEPOINT de #10; solo widening de dos gates de compilación (`TipoAlertaEvaluada`, `TIPOS_MANUALMENTE_RESOLVIBLES`) y un nuevo agregado de 30 días en `MovimientosRepo`. Cobertura: 564 api unit + 525 web unit + 169 integration, todos en verde; verify-report PASS, 0 CRITICAL. Spec (`alertas`) actualizada con 4 requirements ADDED + 1 MODIFIED | #10 | ✅ Archivado |
| 12 | Reportes | Stock actual, bajo mínimo, movimientos por período y discrepancias globales para el encargado; depósito solo reportes operativos con filtrado por dueño (`WHERE usuario_id = :actor`) y sin discrepancias globales; estados vacíos explícitos; paginación. **Cerrado y archivado el 2026-09-04** (PRs #164-#168, todos mergeados a `main`). Cuatro reportes de solo lectura: `ProductosRepo.bajoMinimo`, `MovimientosRepo.listByPeriodo` (primera consulta cross-producto sobre movimientos, requiere `movimientos_fecha_idx`), widening de `AlertasRepo.FiltroAlertas` con `tipo`; primer filtro de autorización a nivel de fila del proyecto (`reportes/service.ts`, `WHERE usuario_id = :actor`, mutado en vivo 3 veces y atrapado por `reportes/service.integration.test.ts`). Migración `0009_brief_paibok.sql` aplicada localmente, requiere `pnpm db:migrate` manual contra Neon antes de usar `/api/reportes/movimientos` en producción — ver `docs/DEPLOY-PLAN.md`. Cobertura: 587 api unit + 545 web unit + 180 integration, todos en verde; verify-report PASS WITH WARNINGS (0 CRITICAL, 1 WARNING resuelto). Spec (`reportes`) promovida como capability nueva | #6, #7 | ✅ Archivado |
| 13 | Dashboard / KPIs | Pantalla de inicio con KPI cards del design.md (quiebres, stock bajo, actividad reciente, alertas activas); chips de estado; navegación por rol con 🔒. **Cerrado y archivado el 2026-09-04** (PRs #170 backend + #171 frontend, ambos mergeados a `main`). 4 KPI cards vía `AlertasRepo.countAbiertasPorTipo` (nuevo, quiebres/stock bajo, no reutiliza `list()`) + `countAbiertas()` (alertas activas, sin filtro de tipo, reutilizado tal cual) + nuevo `MovimientosRepo.listRecientes` (10 más recientes, sin filtrado por actor, reutiliza `movimientos_fecha_idx`); `Panel general` ahora navega a `/` para ambos roles, sin candado. Sin tabla ni migración nueva — cero pasos manuales en Neon, a diferencia de #12. Cobertura: 596 api unit + 561 web unit + 188 integration, todos en verde; verify-report PASS (corregido desde "PASS WITH WARNINGS" en archive — 0 CRITICAL, 0 WARNING, 3 SUGGESTIONS no bloqueantes). Spec (`dashboard-ui`) promovida como capability nueva; deltas mergeados en `app-layout` (MODIFIED) e `inventory-movements` (ADDED) | #6, #10 | ✅ Archivado |
...
Actualizado el 2026-09-01: el ítem **4.1** (Proveedores — vista maestro-detalle) queda archivado.
Pantalla `/proveedores` bajo `shellLayout` (no `encargadoLayout`) con selección vía `?selected=`
(uuid, Zod-validado, `.catch(undefined)`), lista completa filtrable client-side, detalle con
crear/editar/desactivar/reactivar para encargado y vista de solo lectura con motivo 🔒 visible para
depósito, y estado "no encontrado" distinto del placeholder vacío. Integrada en `NAV_ITEMS` de
`AppShell.tsx`. Cobertura: 75 archivos / 488 tests web en verde, `pnpm typecheck` y `pnpm lint`
limpios. Spec (`proveedores-ui`) lista para promoción a `openspec/specs/`.
